/**
 * ==========================================================================
 * TASK MANAGER FRONTEND - JAVASCRIPT LOGIC
 * Codveda Technologies Full-Stack Development Internship - Level 1 Task 3
 * 
 * Target REST API Endpoint: http://localhost:5000/api/tasks
 * Features:
 * - GET: Fetch and display all tasks (getTasks)
 * - POST: Create a new task (createTask)
 * - PATCH: Update task details or status (updateTask)
 * - DELETE: Remove task (deleteTask)
 * ==========================================================================
 */
...
async function getTasks() {
  ...
  try {
    ...
    showError(`Failed to load tasks: ${error.message}. Make sure backend server is running on http://localhost:5000.`);
...
async function createTask(taskData) {
  try {
    ...
    setButtonLoading(submitBtn, false, '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg> Add Task');
  }
}
...
function renderTasks() {
  ...
  if (filteredTasks.length === 0) {
    ...
    if (tasksState.length === 0) {
      emptyStateText.textContent = "You haven't created any tasks yet. Use the form on the left to add your first task!";
    } else {
      emptyStateText.textContent = "No tasks match your current search or filter criteria.";
    }
    return;
  }
...
function updateStats() {
  ...
  const inProgress = tasksState.filter((t) => t.status === 'in-progress').length;
...
function updateServerStatus(isConnected) {
  ...
  serverStatusText.textContent = isConnected ? 'API Connected (v1)' : 'API Offline';
}
